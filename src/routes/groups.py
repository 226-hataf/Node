...
async def list(skip: int = 0, limit: int = 20, db: Session = Depends(get_db),
               user: UserResponseModel = Security(get_current_user, scopes=["groups-list"])):
    """List all the groups"""
    try:
...
async def group(group_name: str, db: Session = Depends(get_db),
                user: UserResponseModel = Security(get_current_user, scopes=["groups-get"])):
    """Get a group by name"""
...
async def update(id: UUIDCheckForIDSchema = Depends(UUIDCheckForIDSchema), groups: GroupBaseSchema = ..., db: Session = Depends(get_db),
...
async def delete(group_name: str, db: Session = Depends(get_db),
                 user: UserResponseModel = Security(get_current_user, scopes=["groups-del"])):
    """Delete a group"""
