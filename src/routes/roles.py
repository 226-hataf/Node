...
# Create Role
@router.post('/', tags=["roles"], status_code=201, response_model=RoleSchema, description="Create a Role")
...
# list all Roles
@router.get('/', tags=["roles"], status_code=200, response_model=list[RoleSchema], description="List all Roles")
...
# get a role
@router.get('/{role_name}', tags=["roles"], status_code=200, response_model=RoleSchema, description="Get a Role")
...
# Update role
@router.put('/{id}', tags=["roles"], status_code=200, description="Update a Role")
async def update(id: UUIDCheckerSchema = Depends(UUIDCheckerSchema), name: str = ..., description: str = ..., db: Session = Depends(get_db)):
    ...
    if not role_exist:
        raise HTTPException(status_code=404, detail="Role not found")
    try:
        updated = crud.update_role(db=db, id=str(checked_uuid), name=name, description=description)
        return updated
...
# Delete role
@router.delete('/{role_name}', tags=["roles"], status_code=202, description="Delete a Role")
